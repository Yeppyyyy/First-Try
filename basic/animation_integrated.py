...
# 更新函数
    ...
    # 如果找的到记录
        ...
        for status in package_statuses:
            nextcoord = location_to_coord[sim.s_or_c(status[1])].pos
            nexttime = status[0]
            if (time - status[0]) <= deltat and (time - status[0]) > 0:
                pretime = status[0]
                precoord = location_to_coord[sim.s_or_c(status[1])].pos
                etat = status[2]
                op = True
            elif time < status[0]:
                break
    # 如果找得到记录
        if op == True:
            ...
            if prestatuses!=[]:
                ...
                precoord2 = location_to_coord[sim.s_or_c(prestatus[1])].pos
