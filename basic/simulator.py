...
class simulator:
    ...
    def pack_deal(self):
        ...
        while pack_Delivered != 0:
            ...
            for loc in self.locations:
                loc.capa = loc.throughput
                loc.buffer['arrived'].sort(key=lambda x: x[1])
                while loc.capa != 0 and loc.buffer['arrived']:
                    pack = loc.buffer['arrived'][0]
                    waiting_time = t - pack[1]
                    pack[1] = pack[1] + waiting_time
            ...
            while (packages_copy):
                if packages_copy[0].time_created <= t:
                    ...
                    self.locations[int(packages_copy[0].src[1:])].pack_passby.put(
                        [packages_copy[0].time_created,packages_copy[0].ID])
    ...
    def tracking(self, idd, time):
        # idd是id,time是时间
        # 用法：print(tr.tracking(sim, idd, data, 15))
        ...
        while i <= len(package_statuses) - 1 and package_statuses[i][0] <= time:
            precoord = location_to_coord[self.s_or_c(package_statuses[i][1])].pos
            ...
            if i <= len(package_statuses) - 2:
                ...
                nextcoord = location_to_coord[self.s_or_c(package_statuses[i + 1][1])].pos
