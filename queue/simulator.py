...
class simulator:
    ...
    def pack_deal(self):
        for pack in self.packages:
            self.tracking_info[pack.ID] = [pack.src, pack.dst, pack.time_created, np.inf]
        ...
        while not self.events.empty() :  # 优先队列不为空
            ...
            if pack_stat[-1] == "ARRIVED":
                ...
                #print(2)
                self.tracking_info[pack.ID].append([time, loc.ID, "SENT"])
                if pack.dst == pack_stat[2]:# 到地方了，就不加回去了 locprocess
                    delivering = delivering -1
                    self.tracking_info[pack.ID][3] = time
